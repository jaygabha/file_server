...
void update_client(int update){
    //Every change in Client no. is communicated to the server
    CLIENT_NO += update;
    char buffer[BUFFER_SIZE] = {0};
    sprintf(buffer, "%d", CLIENT_NO);
    printf("Current Client NO.: %s", buffer);
...
void quit_handler(int sig){
    update_client(-1); //Decrement count of clients
...
int main(int argc, char const* argv[])
{
    ...
    int client_socket;
    //Accept first connection as Original Server Connection
    int count=0;
    while(1)
    {   
        ...
        if(count==0){ 
            //The first connection is the server connection
            count=1;
            server_socket = client_socket; //Set the Server Socket
            CLIENT_NO=0;
            continue;
        }
        ...
        if (pid == -1) {
            ...
            update_client(1); //Increment count of clients.
            printf("No. of Clients Connected: %d\n", CLIENT_NO);
            // Parent process
